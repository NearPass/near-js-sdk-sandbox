...
let CrossCallCounter = (_dec = NearBindgen({}), _dec2 = call({}), _dec(_class = (_class2 = class CrossCallCounter {
  crossCallIncrement({
    contractId
  }) {
    const promise = NearPromise.new(contractId).functionCall("increase", bytes(JSON.stringify({})), BigInt(0), BigInt(10_000_000_000_000));
    return promise.asReturn();
  }

}, (_applyDecoratedDescriptor(_class2.prototype, "crossCallIncrement", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "crossCallIncrement"), _class2.prototype)), _class2)) || _class);
function crossCallIncrement() {
  let _state = CrossCallCounter._getState();

  if (!_state && CrossCallCounter._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = CrossCallCounter._create();

  if (_state) {
    CrossCallCounter._reconstruct(_contract, _state);
  }

  let _args = CrossCallCounter._getArgs();

  let _result = _contract.crossCallIncrement(_args);

  CrossCallCounter._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CrossCallCounter._serialize(_result));
}

export { CrossCallCounter, crossCallIncrement };
