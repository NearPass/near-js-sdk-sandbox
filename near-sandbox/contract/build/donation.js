...
let DonationContract = (_dec = NearBindgen({}), _dec2 = initialize({}), _dec3 = call({
  ...
  donate() {
    ...
    let donatedSoFar = this.donations.get(donor) === null ? BigInt(0) : BigInt(this.donations.get(donor));
    ...
    log(`Thank you ${donor} for donating ${donationAmount}! You donated a total of ${donatedSoFar}`); // Send the money to the beneficiary
...
function number_of_donors() {
  ...
  let _result = _contract.number_of_donors(_args);
...
function change_beneficiary() {
  ...
  let _result = _contract.change_beneficiary(_args);
...
function donate() {
  ...
  let _result = _contract.donate(_args);
