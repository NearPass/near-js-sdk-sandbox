...
let MapTest = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view({}), _dec(_class = (_class2 = class MapTest {
...
}, (_applyDecoratedDescriptor(_class2.prototype, "set", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "set"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "get"), _class2.prototype)), _class2)) || _class);
function get() {
  let _state = MapTest._getState();

  if (!_state && MapTest._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = MapTest._create();

  if (_state) {
    MapTest._reconstruct(_contract, _state);
  }

  let _args = MapTest._getArgs();

  let _result = _contract.get(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(MapTest._serialize(_result));
}
function set() {
  let _state = MapTest._getState();
  ...
  let _result = _contract.set(_args);

  MapTest._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(MapTest._serialize(_result));
}

export { MapTest, get, set };
